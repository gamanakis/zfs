...
static int scrub_clones(spa_t *spa, uint64_t zap_clone, uint64_t snap_count,
    uint64_t *snap_obj_array, zbookmark_err_phys_t *zep, int *count);
 ...
 * when deciding whether we exceeded zfs_scrub_error_blocks_in_one_txg.
 */
static int
    ...
    int *count)
{
	...
	if (zep->zb_birth == 0) {
		...
		zbookmark_phys_t zb;
		zep_to_zb(fs, zep, &zb);
		spa_log_error(spa, &zb, &zep->zb_birth);
		dsl_dataset_rele(ds, FTAG);
		return (SET_ERROR(EINVAL));
	}
	 ...
	 * proceed. In worst case scenario scrub if unnecessary.
	 */
	...
	if (zap_clone != 0 && aff_snap_count > 0) {
		error = scrub_clones(spa, zap_clone, snap_count, snap_obj_array,
		    zep, count);
	}
    ...
    uint64_t *snap_obj_array, zbookmark_err_phys_t *zep, int *count)
{
	...
	for (zap_cursor_init(zc, spa->spa_meta_objset, zap_clone);
		...
		error = scrub_filesystem(spa, za->za_first_integer, zep,
		    count);
